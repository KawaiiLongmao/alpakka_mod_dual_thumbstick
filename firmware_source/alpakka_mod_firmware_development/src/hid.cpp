...
int16_t mouse_x = 0;
int16_t mouse_y = 0;
double gamepad_axis[6] = {
    0,
};
double gamepad_axis_last[6] = {
    0,
};
...
static bool report_was_sent[4] = {
    false,
}; // Prevent replay if no report was ever sent.
static uint8_t cycles_without_reporting[4] = {
    0,
}; // Cycles since the last report.
static uint8_t replayed_ntimes[4] = {
    0,
}; // How many times the last report was replayed.
...
void hid_press_multiple(uint8_t *keys)
{
    ...
    if (keys[0] == PROC_HOME)
        return; // Home does not count as input.
...
{
    for (uint8_t i = 0; i < 6; i++)
        gamepad_axis_last[i] = gamepad_axis[i];
    synced_gamepad = true;
    priority_gamepad = 0;
}
...
void hid_report_keyboard(bool wired)
{
    ...
    if (wired)
        ...
        wireless_send_hid(REPORT_KEYBOARD, &report, sizeof(report));
...
void hid_report_mouse(bool wired)
{
    ...
    if (wired)
        ...
        wireless_send_hid(REPORT_MOUSE, &report, sizeof(report));
    hid_reset_mouse();
    synced_mouse = true;
    priority_mouse = 0;
...
{
    if (type == REPORT_XINPUT)
        type = REPORT_GAMEPAD; // Gamepad and Xinput counter is shared.
...
ReportType hid_get_priority()
{
    ...
    if (!synced_mouse)
        priority_mouse += 1 * HID_REPORT_PRIORITY_RATIO;
    if (!synced_gamepad)
        priority_gamepad += 1;
    // Replay.
    if (synced_keyboard && hid_should_replay(REPORT_KEYBOARD))
        return REPORT_REPLAY_KEYBOARD;
    if (synced_mouse && hid_should_replay(REPORT_MOUSE))
        return REPORT_REPLAY_MOUSE;
    if (synced_gamepad && hid_should_replay(REPORT_GAMEPAD))
    {
        if (config_get_protocol() == PROTOCOL_GENERIC)
            return REPORT_REPLAY_GAMEPAD;
        else
            return REPORT_REPLAY_XINPUT;
    }
    ...
    {
        if (config_get_protocol() == PROTOCOL_GENERIC)
            return REPORT_GAMEPAD;
        else
            return REPORT_XINPUT;
    }
    return (ReportType)0;
}
...
bool hid_report_wired()
{
    ...
    if (tud_ready())
    {
        if (tud_hid_ready())
        {
            webusb_read();
            webusb_flush();
            if (device_to_report == REPORT_KEYBOARD)
                hid_report_keyboard(true);
            if (device_to_report == REPORT_MOUSE)
                hid_report_mouse(true);
            if (device_to_report == REPORT_GAMEPAD)
                hid_report_gamepad(true);
        }
...
bool hid_report_wireless()
{
    ...
    ReportType device_to_report = hid_get_priority();
    if (device_to_report == REPORT_KEYBOARD)
        hid_report_keyboard(false);
    if (device_to_report == REPORT_MOUSE)
        hid_report_mouse(false);
    if (device_to_report == REPORT_GAMEPAD)
        hid_report_gamepad(false);
    if (device_to_report == REPORT_XINPUT)
        hid_report_xinput(false);
    // Replay.
    if (device_to_report == REPORT_REPLAY_KEYBOARD)
        hid_replay_keyboard();
    if (device_to_report == REPORT_REPLAY_MOUSE)
        hid_replay_mouse();
    if (device_to_report == REPORT_REPLAY_GAMEPAD)
        hid_replay_gamepad();
    if (device_to_report == REPORT_REPLAY_XINPUT)
        hid_replay_xinput();
    // Update replay state.
    if (device_to_report <= REPORT_XINPUT)
    { // Skip update when a report is being replayed.
...
#include "vector.h"
Vector g_gamepad_gyro = {};
Vector g_gamepad_accel = {};
