...
void gyro_accel_correction()
{
    ...
    g_imu_accel.x /= -BIT_14;
    g_imu_accel.y /= -BIT_14;
    g_imu_accel.z /= -BIT_14;
    // Get a smoothed gravity vector.
    accel_smooth = vector_smooth(accel_smooth, g_imu_accel, CFG_ACCEL_CORRECTION_SMOOTH);
...
void Gyro__report_absolute(Gyro *self)
{
    ...
    Vector4 rx = quaternion(world_right, g_imu_gyro.y / sens);
    Vector4 ry = quaternion(world_fw, g_imu_gyro.z / sens);
    Vector4 rz = quaternion(world_top, g_imu_gyro.x / sens);
    static uint8_t i = 0;
    Vector4 r;
    if (i == 0)
...
void Gyro__report_incremental(Gyro *self)
{
    ...
    double x = g_imu_gyro.x * CFG_GYRO_SENSITIVITY_X * sensitivity_multiplier;
    double y = g_imu_gyro.y * CFG_GYRO_SENSITIVITY_Y * sensitivity_multiplier;
    double z = g_imu_gyro.z * CFG_GYRO_SENSITIVITY_Z * sensitivity_multiplier;
...
{
    g_imu_accel = imu_read_accel();
    g_imu_gyro = imu_read_gyro();
    g_gamepad_accel = g_imu_accel;
    if (touch_status())
    {
        g_gamepad_gyro = g_imu_gyro;
    }
    else
    {
        g_gamepad_gyro = {};
