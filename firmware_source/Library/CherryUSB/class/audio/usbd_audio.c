...
static struct usbd_audio_request_callback s_request_callback = {};

static void usbd_audio_open(uint8_t busid, uint8_t intf)
{
    (void)busid;
    (void)intf;
}

static void usbd_audio_close(uint8_t busid, uint8_t intf)
{
    (void)busid;
    (void)intf;
}

static void usbd_audio_set_volume(uint8_t busid, uint8_t ep, uint8_t ch, int volume_db)
{
    (void)busid;
    (void)ep;
    (void)ch;
...
static int usbd_audio_get_volume(uint8_t busid, uint8_t ep, uint8_t ch)
{
    (void)busid;
    (void)ep;
    (void)ch;
...
static void usbd_audio_set_mute(uint8_t busid, uint8_t ep, uint8_t ch, bool mute)
{
    (void)busid;
    (void)ep;
    (void)ch;
...
static bool usbd_audio_get_mute(uint8_t busid, uint8_t ep, uint8_t ch)
{
    (void)busid;
    (void)ep;
    (void)ch;
...
static void usbd_audio_set_sampling_freq(uint8_t busid, uint8_t ep, uint32_t sampling_freq)
{
    (void)busid;
    (void)ep;
    (void)sampling_freq;
...
static uint32_t usbd_audio_get_sampling_freq(uint8_t busid, uint8_t ep)
{
    (void)busid;
    (void)ep;
...
static void usbd_audio_get_sampling_freq_table(uint8_t busid, uint8_t ep, uint8_t **sampling_freq_table)
{
    (void)busid;
    (void)ep;
...
static int audio_class_endpoint_request_handler(uint8_t busid, struct usb_setup_packet *setup, uint8_t **data, uint32_t *len)
{
    ...
    case AUDIO_EP_CONTROL_SAMPLING_FEQ:
        ...
        case AUDIO_REQUEST_SET_CUR:
            ...
            s_request_callback.usbd_audio_set_sampling_freq(busid, ep, sampling_freq);
            break;
        case AUDIO_REQUEST_GET_CUR:
        case AUDIO_REQUEST_GET_MIN:
        case AUDIO_REQUEST_GET_MAX:
        case AUDIO_REQUEST_GET_RES:
            sampling_freq = s_request_callback.usbd_audio_get_sampling_freq(busid, ep);
    ...
    case AUDIO_CONTROL_FEATURE_UNIT:
        ...
        case AUDIO_FU_CONTROL_MUTE:
            if (g_usbd_audio[busid].uac_version < 0x0200)
            {
                ...
                case AUDIO_REQUEST_SET_CUR:
                    mute = (*data)[0];
                    s_request_callback.usbd_audio_set_mute(busid, ep, ch, mute);
                    break;
                case AUDIO_REQUEST_GET_CUR:
                    (*data)[0] = s_request_callback.usbd_audio_get_mute(busid, ep, ch);
                    *len = 1;
                    break;
                default:
        ...
        case AUDIO_FU_CONTROL_VOLUME:
            if (g_usbd_audio[busid].uac_version < 0x0200)
            {
                ...
                case AUDIO_REQUEST_SET_CUR:
                    ...
                    s_request_callback.usbd_audio_set_volume(busid, ep, ch, volume_db);
                    break;
                case AUDIO_REQUEST_GET_CUR:
                    volume_db = s_request_callback.usbd_audio_get_volume(busid, ep, ch);
                    if (volume_db >= 0)
                    {
        ...
        case AUDIO_CS_CONTROL_SAM_FREQ:
            ...
            case AUDIO_REQUEST_RANGE:
                if (setup->bmRequestType & USB_REQUEST_DIR_MASK)
                {
                    ...
                    s_request_callback.usbd_audio_get_sampling_freq_table(busid, ep, &sampling_freq_table);
...
static void audio_notify_handler(uint8_t busid, uint8_t event, void *arg)
{
    ...
    case USBD_EVENT_SET_INTERFACE:
    {
        ...
        {
            s_request_callback.usbd_audio_open(busid, intf->bInterfaceNumber);
        }
        else
        {
            s_request_callback.usbd_audio_close(busid, intf->bInterfaceNumber);
        }
    }
...
struct usbd_interface *usbd_audio_init_intf(uint8_t busid,
                                            ...
                                            struct audio_entity_info *table,
                                            uint8_t num,
                                            struct usbd_audio_request_callback cb)
{
    ...
    s_request_callback.usbd_audio_open = cb.usbd_audio_open ? cb.usbd_audio_open : usbd_audio_open;
    s_request_callback.usbd_audio_close = cb.usbd_audio_close ? cb.usbd_audio_close : usbd_audio_close;

    s_request_callback.usbd_audio_set_volume = cb.usbd_audio_set_volume ? cb.usbd_audio_set_volume : usbd_audio_set_volume;
    s_request_callback.usbd_audio_get_volume = cb.usbd_audio_get_volume ? cb.usbd_audio_get_volume : usbd_audio_get_volume;
    s_request_callback.usbd_audio_set_mute = cb.usbd_audio_set_mute ? cb.usbd_audio_set_mute : usbd_audio_set_mute;
    s_request_callback.usbd_audio_get_mute = cb.usbd_audio_get_mute ? cb.usbd_audio_get_mute : usbd_audio_get_mute;

    s_request_callback.usbd_audio_set_sampling_freq = cb.usbd_audio_set_sampling_freq ? cb.usbd_audio_set_sampling_freq : usbd_audio_set_sampling_freq;
    s_request_callback.usbd_audio_get_sampling_freq = cb.usbd_audio_get_sampling_freq ? cb.usbd_audio_get_sampling_freq : usbd_audio_get_sampling_freq;
    s_request_callback.usbd_audio_get_sampling_freq_table = cb.usbd_audio_get_sampling_freq_table ? cb.usbd_audio_get_sampling_freq_table : usbd_audio_get_sampling_freq_table;
