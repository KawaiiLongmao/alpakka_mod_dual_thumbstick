...
#if MOD_LINGHU
    self->lk.report(&self->lk);
    self->rk.report(&self->rk);
#else
...
#if MOD_LINGHU
    self->lk.reset(&self->lk);
    self->rk.reset(&self->rk);
#else
...
void profile_notify_protocol_changed(Protocol protocol)
{
    ...
    profile_protocol_was_changed = (Protocol)((int8_t)protocol);
}
...
void profile_init()
{
    ...
    home = Button_(PIN_HOME, (ButtonMode)(DOUBLE | IMMEDIATE), actions, actions_secondary, actions_terciary);
    // Profiles setup.
    for (uint8_t i = 0; i < PROFILE_SLOTS; i++)
    {
        profiles[i] = Profile_();
