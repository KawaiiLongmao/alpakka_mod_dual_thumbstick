...
bool config_profile_cache_synced[NVM_PROFILE_SLOTS] = {
    0,
};
...
void config_profile_load(uint8_t index)
{
    ...
    uint32_t version = ((meta.version_major * 1000000) +
...
{
    if (profile == config_cache.profile)
        return;
...
void config_tune_update_leds()
{
    if (config_tune_mode == PROC_TUNE_OS)
    {
        ...
        if (config_cache.protocol == 0)
            led_blink_mask(LED_LEFT);
        if (config_cache.protocol == 1)
            led_blink_mask(LED_DOWN);
        if (config_cache.protocol == 2)
            led_blink_mask(LED_RIGHT);
        if (config_cache.protocol == 3)
            led_blink_mask(LED_UP);
        if (config_cache.protocol == 4)
            led_blink_mask(LED_LEFT + LED_UP);
        if (config_cache.protocol == 5)
            led_blink_mask(LED_DOWN + LED_LEFT);
        if (config_cache.protocol == 6)
    ...
    if (config_tune_mode == PROC_TUNE_MOUSE_SENS)
    {
        ...
        if (config_cache.sens_mouse == 0)
            led_blink_mask(LED_LEFT);
        if (config_cache.sens_mouse == 1)
            led_blink_mask(LED_UP);
        if (config_cache.sens_mouse == 2)
            led_blink_mask(LED_RIGHT);
        led_set_mode(LED_MODE_BLINK);
    }
    if (config_tune_mode == PROC_TUNE_DEADZONE)
    {
        ...
        if (config_cache.deadzone == 0)
            led_blink_mask(LED_DOWN);
        if (config_cache.deadzone == 1)
            led_blink_mask(LED_RIGHT);
        if (config_cache.deadzone == 2)
            led_blink_mask(LED_UP);
        led_set_mode(LED_MODE_BLINK);
    }
...
void config_tune(bool direction)
{
    ...
    if (config_tune_mode == PROC_TUNE_OS)
    {
        config_set_protocol(constrain(config_cache.protocol + value, 0, PROTOCOL_MAX));
    }
...
bool current_protocol_compatible_with_webusb()
{
    ...
    return p == PROTOCOL_XUSB_WIN || p == PROTOCOL_XUSB_UNIX || p == PROTOCOL_GENERIC;
    // return true;
}
...
{
    uint8_t p = config_get_protocol();
    return p == PROTOCOL_GENERIC;
}

bool config_current_protocol_exist_gyro()
{
    uint8_t p = config_get_protocol();
    return p == PROTOCOL_SWITCH_PRO || p == PROTOCOL_DUAL_SHOCK_4 || p == PROTOCOL_DUAL_SENSE;
}
...
{
    if (preset == config_cache.protocol)
        return;
...
void config_set_mouse_sens_preset(uint8_t preset, bool notify_webusb)
{
    ...
    if (notify_webusb)
        webusb_set_pending_config_share(SENS_MOUSE);
...
void config_set_deadzone_preset(uint8_t preset, bool notify_webusb)
{
    ...
    if (notify_webusb)
        webusb_set_pending_config_share(DEADZONE);
...
{
    if (!config_problems_are_pending())
        return;
...
void config_profile_default(uint8_t indexTo, int8_t indexFrom)
{
    ...
    if (indexFrom == 0)
        config_profile_default_home(&(config_profile_cache[indexTo]));
    if (indexFrom == 1)
        config_profile_default_fps_fusion(&(config_profile_cache[indexTo]));
    if (indexFrom == 2)
        config_profile_default_racing(&(config_profile_cache[indexTo]));
    if (indexFrom == 3)
        config_profile_default_console(&(config_profile_cache[indexTo]));
    if (indexFrom == 4)
        config_profile_default_desktop(&(config_profile_cache[indexTo]));
    if (indexFrom == 5)
        config_profile_default_fps_wasd(&(config_profile_cache[indexTo]));
    if (indexFrom == 6)
        config_profile_default_flight(&(config_profile_cache[indexTo]));
    if (indexFrom == 7)
        config_profile_default_console_legacy(&(config_profile_cache[indexTo]));
    if (indexFrom == 8)
        config_profile_default_rts(&(config_profile_cache[indexTo]));
    if (indexFrom == 9)
        config_profile_default_custom(&(config_profile_cache[indexTo]));
    if (indexFrom == 10)
        config_profile_default_custom(&(config_profile_cache[indexTo]));
    if (indexFrom == 11)
        config_profile_default_custom(&(config_profile_cache[indexTo]));
