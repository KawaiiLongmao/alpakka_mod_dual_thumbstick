...
static uint16_t read_filter_adc()
{
    static const uint8_t SAMPLES = 6;
    int vals[SAMPLES];
    for (int i = 0; i < SAMPLES; i++)
    {
        vals[i] = adc_read();
    }
    return calculate_trimmed_mean(vals, SAMPLES);
}
...
{
    if (!thumbstick_smooth_samples)
        return thumbstick_adc(pin);
    uint8_t channel = pin - PIN_ADC_FIRST;
...
void thumbstick_calibrate_each(uint8_t pin_x, uint8_t pin_y, float *result_x, float *result_y)
{
    ...
    for (uint32_t i = 0; i < nsamples; i++)
    {
        ...
        if (!(i % (nsamples / CFG_CALIBRATION_PROGRESS_BAR)))
            info("=");
    }
...
void thumbstick_report_axis(uint8_t axis, float value)
{
    if (axis == GAMEPAD_AXIS_LX)
        hid_gamepad_axis(LX, value);
    else if (axis == GAMEPAD_AXIS_LY)
        hid_gamepad_axis(LY, value);
    else if (axis == GAMEPAD_AXIS_RX)
        hid_gamepad_axis(RX, value);
    else if (axis == GAMEPAD_AXIS_RY)
        hid_gamepad_axis(RY, value);
        ...
        hid_gamepad_axis(LZ, value);
    else if (axis == GAMEPAD_AXIS_RZ)
        hid_gamepad_axis(RZ, value);
}
...
void thumbstick_from_ctrl(Thumbstick *thumbstick, CtrlProfile *ctrl, uint8_t index)
{
    ...
    *thumbstick = Thumbstick_(
        ...
        (ThumbstickDistance)ctrl_thumbtick.distance_mode,
...
void thumbstick_report_mouse_move(uint8_t action, float thumbstick_value, uint8_t response_curve, uint8_t sensitivity_level, uint8_t exponent_level)
{
    if (response_curve < 1 || response_curve > 3)
        response_curve = LINEAR;
    if (sensitivity_level < 1 || sensitivity_level > 10)
        sensitivity_level = 1;
    if (exponent_level < 1 || exponent_level > 10)
        exponent_level = 5;

    static const float REGULAR_VALUE = 2.0f;
    float mouse_move_value = 0;
    switch (response_curve)
    {
    case LINEAR:
    {
        float sensitivity = 1.0f + (sensitivity_level - 1) * 0.5f;
        mouse_move_value = thumbstick_value * sensitivity;
        break;
    }
    case TRADITIONAL_CURVE:
    {
        float sensitivity = 1.0f + (sensitivity_level - 1) * 0.5f;
        float exponent = 1.5f + (sensitivity_level - 1) * 0.1f;
        mouse_move_value = powf(thumbstick_value, exponent);
        mouse_move_value *= sensitivity;
        break;
    }
    case CONSTANT:
    {
        ...
        {
            mouse_move_value = sensitivity_level * 0.2f;
        }
        break;
    }
    ...
    mouse_move_value *= REGULAR_VALUE;

    double rpt_value = constrain(mouse_move_value, 0, BIT_7);
    if (action == MOUSE_X)
        hid_mouse_move(rpt_value, 0);
    else if (action == MOUSE_Y)
        hid_mouse_move(0, rpt_value);
...
void Thumbstick__report_4dir_axial(Thumbstick *self, ThumbstickPosition pos)
{
    ...
    if (hid_is_mouse_move(self->left.actions[0]))
    {
        thumbstick_report_mouse_move(self->left.actions[0],
                                     -constrain(pos.x, -1, 0),
                                     self->ul.actions[0] - KEY_Z,
                                     self->dl.actions[0] - KEY_Z,
                                     self->ur.actions[0] - KEY_Z);
        report_mouse_move = true;
    }
    //// Right.
    if (hid_is_mouse_move(self->right.actions[0]))
    {
        thumbstick_report_mouse_move(self->right.actions[0],
                                     constrain(pos.x, 0, 1),
                                     self->ul.actions[0] - KEY_Z,
                                     self->dl.actions[0] - KEY_Z,
                                     self->ur.actions[0] - KEY_Z);
    ...
    //// Up.
    if (hid_is_mouse_move(self->up.actions[0]))
    {
        thumbstick_report_mouse_move(self->up.actions[0],
                                     -constrain(pos.y, -1, 0),
                                     self->ul.actions[0] - KEY_Z,
                                     self->dr.actions[0] - KEY_Z,
                                     self->ur.actions[0] - KEY_Z);
        report_mouse_move = true;
    }
    //// Down.
    if (hid_is_mouse_move(self->down.actions[0]))
    {
        thumbstick_report_mouse_move(self->down.actions[0],
                                     constrain(pos.y, 0, 1),
                                     self->ul.actions[0] - KEY_Z,
                                     self->dr.actions[0] - KEY_Z,
                                     self->ur.actions[0] - KEY_Z);
    ...
    if (!report_mouse_move)
    {
        ...
        {
            if (pos.radius < THUMBSTICK_INNER_RADIUS)
                self->inner.virtual_press = true;
            else
                self->outer.virtual_press = true;
        ...
        if (!hid_is_axis(self->left.actions[0]))
            ...
            thumbstick_report_axis(self->left.actions[0], -constrain(pos.x, -1, 0));
        //// Right.
        if (!hid_is_axis(self->right.actions[0]))
            self->right.report(&self->right);
        else
            thumbstick_report_axis(self->right.actions[0], constrain(pos.x, 0, 1));
        //// Up.
        if (!hid_is_axis(self->up.actions[0]))
            self->up.report(&self->up);
        else
            thumbstick_report_axis(self->up.actions[0], -constrain(pos.y, -1, 0));
        //// Down.
        if (!hid_is_axis(self->down.actions[0]))
            self->down.report(&self->down);
        else
            thumbstick_report_axis(self->down.actions[0], constrain(pos.y, 0, 1));
    }
...
void Thumbstick__report_8dir(Thumbstick *self, ThumbstickPosition pos)
{
    ...
    if (!report_mouse_move)
    {
        ...
        if (pos.radius > THUMBSTICK_ADDITIONAL_DEADZONE_FOR_BUTTONS)
        {
            ...
            if (direction == DIR4_MASK_LEFT)
                self->left.virtual_press = true;
            else if (direction == DIR4_MASK_RIGHT)
                self->right.virtual_press = true;
            else if (direction == DIR4_MASK_UP)
                self->up.virtual_press = true;
            else if (direction == DIR4_MASK_DOWN)
                self->down.virtual_press = true;
            else if (direction == (DIR4_MASK_UP + DIR4_MASK_LEFT))
                self->ul.virtual_press = true;
            else if (direction == (DIR4_MASK_UP + DIR4_MASK_RIGHT))
                self->ur.virtual_press = true;
            else if (direction == (DIR4_MASK_DOWN + DIR4_MASK_LEFT))
                self->dl.virtual_press = true;
            else if (direction == (DIR4_MASK_DOWN + DIR4_MASK_RIGHT))
                self->dr.virtual_press = true;
        }
...
{
    dir = (Dir8)((int)dir - 1); // Shift zero since not using center direction here.
...
void Thumbstick__report_alphanumeric(Thumbstick *self, ThumbstickPosition pos)
{
    ...
    if (pos.radius > 0.7)
    {
        ...
        // Detect direction 4.
        if (is_between(pos.angle, -CUT4X, -CUT4))
            dir4 = DIR4_LEFT;
        else if (is_between(pos.angle, CUT4, CUT4X))
            dir4 = DIR4_RIGHT;
        else if (fabs(pos.angle) <= 90 - CUT4)
            dir4 = DIR4_UP;
        else if (fabs(pos.angle) >= 90 + CUT4)
            dir4 = DIR4_DOWN;
        // Detect direction 8.
        if (is_between(pos.angle, -CUT8 * 1, CUT8 * 1))
            dir8 = DIR8_UP;
        else if (is_between(pos.angle, CUT8 * 1, CUT8 * 3))
            dir8 = DIR8_UP_RIGHT;
        else if (is_between(pos.angle, CUT8 * 3, CUT8 * 5))
            dir8 = DIR8_RIGHT;
        else if (is_between(pos.angle, CUT8 * 5, CUT8 * 7))
            dir8 = DIR8_DOWN_RIGHT;
        else if (is_between(pos.angle, -CUT8 * 7, -CUT8 * 5))
            dir8 = DIR8_DOWN_LEFT;
        else if (is_between(pos.angle, -CUT8 * 5, -CUT8 * 3))
            dir8 = DIR8_LEFT;
        else if (is_between(pos.angle, -CUT8 * 3, -CUT8 * 1))
            dir8 = DIR8_UP_LEFT;
        else if (fabs(pos.angle) >= CUT8 * 7)
            dir8 = DIR8_DOWN;
        // Record direction 4.
